...
export default async function Home({ params }: { params: { locale: string } }) {
	...
	const rawStoryVideos = await getStoryVideos(locale)
	const storyVideos = rawStoryVideos.map(story => ({
		...story,
		thumbnail: (story as any).thumbnail ?? { id: '', url: '' },
	}))
	const rawBlogPosts = await getBlogPosts(locale)
			...
			<div className=' relative'>
				...
				>
					...
					<path
						...
						opacity='.5'
						className='fill-indigo-500  delay-500'
					></path>
					<path
						d='M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z'
						className='fill-purple-500  delay-1000'
					></path>
				</svg>
			</div>
			{/* Blog Section */}
